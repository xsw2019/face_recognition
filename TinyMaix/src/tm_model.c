...
tm_err_t TM_WEAK tm_preprocess(tm_mdl_t* mdl, tm_pp_t pp_type, tm_mat_t* in, tm_mat_t* out)
{
    ...
    sctype_t in_s = l0h->in_s;//sctype_t means float 
    zptype_t in_zp= l0h->in_zp;//zeropoint data type 
    int in_size = in->h*in->w*in->c;//length in 3 dims
    switch(pp_type){
    ...
    case TMPP_UINT2INT:
        for(int i=0; i<in_size; i++)
            out->data[i] = ((mtype_t)(((uint8_t*)(in->data))[i]-128))<<UINT2INT_SHIFT;//0~256 to -128~127
						//out->data[i] = ((mtype_t)(((uint8_t*)(in->data))[i]));//0~256 to -128~127
        break;
#else
    case TMPP_UINT2FP01:
        for(int i=0; i<in_size; i++)
